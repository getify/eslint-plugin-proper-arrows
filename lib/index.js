	...
	rules: {
		...
		"this": {
			meta: {
				...
				schema: [
					{
						enum: [ "allow", "always", "nested", "never", ],
					},
					{
						type: "object",
						properties: {
			...
			create(context) {
				...
				var allowThis = (context.options[0] === "allow");
				var alwaysThis = (context.options[0] === "always");
				var neverThis = (context.options[0] === "never");
				var noGlobal = (
					["always","nested","allow",].includes(context.options[0]) &&
					context.options[1] &&
				...
				return {
					...
					"ArrowFunctionExpression:exit": function exit(node) {
						...
						else if (!allowThis && !foundThis && !neverThis) {
